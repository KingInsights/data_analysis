...
def chart_1(chart_type):
    ...
    if chart_type == "Bar":
        sns.barplot(x=avg_price.values, y=avg_price.index, ax=ax, color=color_themes[theme]["primary"])
...
def chart_2(chart_type):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    if chart_type == "Bar":
        sns.barplot(x=max_price.values, y=max_price.index, ax=ax, color=color_themes[theme]["primary"])
    elif chart_type == "Line":
...
def chart_3(chart_type):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    if chart_type == "Bar":
        sns.barplot(x=min_price.values, y=min_price.index, ax=ax, color=color_themes[theme]["primary"])
    elif chart_type == "Line":
...
def chart_4(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x="units_sold_12m", y="product_name", data=top_units, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_5(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=revenue.values, y=revenue.index, ax=ax, color=color_themes[theme]["primary"])
        else:
        ...
        ax.pie(revenue.values, labels=revenue.index, autopct="%1.1f%%",
               colors=[color_themes[theme]["primary"]]*len(revenue))
...
def chart_6(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=stock_value.values, y=stock_value.index, ax=ax, color=color_themes[theme]["primary"])
        else:
        ...
        ax.pie(stock_value.values, labels=stock_value.index, autopct="%1.1f%%",
               colors=[color_themes[theme]["primary"]]*len(stock_value))
...
def chart_7(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=avg_discount.values, y=avg_discount.index, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_8(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        ...
        if chart_type == "Bar":
            sns.barplot(x=stock_counts.index, y=stock_counts.values, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_9(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x="bulk_price_per_unit", y="product_name", data=best_bulk, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_10(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=size_counts.index, y=size_counts.values, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_11(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=avg_weight.values, y=avg_weight.index, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_12(chart_type):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    if chart_type == "Bar":
        sns.histplot(prices, bins=30, kde=False, ax=ax, color=color_themes[theme]["primary"])
        ax.set_title("Product Price Distribution (Histogram)")
    elif chart_type == "Line":
        sns.kdeplot(prices, ax=ax, color=color_themes[theme]["primary"], fill=True)
        ax.set_title("Product Price Distribution (KDE Curve)")
    elif chart_type == "Pie":
...
def chart_13(chart_type):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    if chart_type == "Line":
        sns.lineplot(x=product_revenue.index + 1, y="cum_pct", data=product_revenue, ax=ax, color=color_themes[theme]["primary"])
...
def chart_14(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x="discount_amount_pct", y="product_name", data=top_discount, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_15(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=cheapest_by_size.index, y=cheapest_by_size.values, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_16(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=review_counts.values, y=review_counts.index, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_18(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x="units_sold_12m", y="product_name", data=value_hits, ax=ax, color=color_themes[theme]["primary"])
        else:
...
def chart_19(chart_type):
    ...
    if chart_type in ["Bar", "Line"]:
        if chart_type == "Bar":
            sns.barplot(x=high_price_counts.values, y=high_price_counts.index, ax=ax, color=color_themes[theme]["primary"])
        else:
...
if st.session_state.show_sentiment_chart_1:
    ...
    if chart_type == "Bar":
        ...
        ax.pie(
            top_happy["avg_sentiment"],
            labels=top_happy["product_name"],
            autopct="%1.1f%%",
            colors=["#1ABC9C"] * len(top_happy)
        )
        ax.set_aspect("equal")
...
if st.button("Show Top 10 Worst-Rated Products by Sentiment"):
    ...
    sns.barplot(
        x="avg_sentiment",  # Sentiment score on x-axis
        y="product_name",   # Product names on y-axis
        data=top_unhappy,   # Data to use
        color="#E74C3C",     # Color for the bars
        errorbar=None       # Removes the black error bars
    )
...
if st.button("Show Average Sentiment by Manufacturer"):
    ...
    plt.figure(figsize=(10, 6))
    sns.barplot(
        x=avg_sentiment_manufacturer.values,  # Sentiment scores
        y=avg_sentiment_manufacturer.index,   # Manufacturers
        color="#3498DB",  # solid blue
        errorbar=None     # Removes the black confidence interval lines
    )
